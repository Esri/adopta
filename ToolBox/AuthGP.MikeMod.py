...
def email_exists(input_email, check_token_validity=False):
    ...
    send_msg("Whereclause: {0}".format(str(where_clause)), "verbose")
    ...
    try:
        with arcpy.da.SearchCursor(in_table=user_table,
            ...
            send_msg("Search Successful", "verbose")

            rowcount = len([i for i in cursor])
            send_msg("Rowcount: {0}".format(str(rowcount)), "verbose")
...
def update_usertoken(expired_email=""):
    ...
    try:
        ...
        datasetVersioned = arcpy.Describe(user_table).isVersioned
        send_msg("Data versioning. {0}".format(str(datasetVersioned )), "warning")
        if datasetVersioned:
...
def add_user():
    ...
    send_msg("Fields: {0}".format(str(fields)), "verbose")
    # add email field
    ...
    send_msg("Fields Updated: {0}".format(str(fields)), "verbose")
    ...
    try:
        ...
        send_msg("Workspace: {0}".format(str(wksp)), "verbose")
        send_msg("Table: {0}".format(str(user_table)), "verbose")

        canVersion = desc.canVersion
        send_msg("canVersion: {0}".format(str(canVersion)), "verbose")

        datasetVersioned = desc.isVersioned
        send_msg("isVersioned: {0}".format(desc.isVersioned), "verbose")
        ...
        if datasetVersioned == True:
            ...
            edit.startEditing(False, True)
        send_msg("Editing Started", "verbose")

        # Start an edit operation
        edit.startOperation()
        send_msg("Operation Started", "verbose")
        # insert row for new user
        with arcpy.da.InsertCursor(in_table=user_table,
                                   field_names=fields.keys()) as cursor:
            send_msg("Cursor Created", "verbose")
            try:
                oid = cursor.insertRow(fields.values())
            except Exception as e:
                send_msg("Error inserting: {0}".format(str(e), "verbose"))
                if edit.isEditing:
                    edit.stopOperation()
                    edit.stopEditing(False)
                edit.startEditing(False, True)
                edit.startOperation()
                oid = cursor.insertRow(fields.values())
            send_msg("Row inserted: {0}".format(oid), "verbose")
...
def process_signup():
    ...
    send_msg("Email Status {0}".format(str(email_status)), "verbose")
    if email_status is True:
        send_msg("Email already exists.", "error")
        return
    elif email_status is False:
        # add user to database
        send_msg("Trying to add the user.", "verbose")
...
def main():
    ...
    send_msg("Validate Inputes", "verbose")
    if not validate_inputs():
        return
    send_msg("Validate user table", "verbose")
    # validate user table schema
    if not validate_user_table():
        return
    send_msg(action, "verbose")
    # return unique team names
    if action.lower() == "teams":
